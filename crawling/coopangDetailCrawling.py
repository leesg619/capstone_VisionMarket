...
for page_number in range(1, maxpage+1) :
    ...
    detailPages = []
    for i in products :
        detailPages.append(i.a.attrs['href'])


    for detailPage in detailPages :
        temp = []
        reqUrl = requests.get('https://www.coupang.com'+detailPage, headers={'User-Agent': 'Mozilla/5.0'})
        soupDetail = BeautifulSoup(reqUrl.content, 'html.parser')
        
        content = soupDetail.select_one('#contents')
        # print(content)
        mainImage = content.select_one('.prod-image__detail').attrs['src']
        title = content.select_one('.prod-buy-header__title').text
        originalPrice = content.select_one('.total-price').text
        tempSizeAll = []
        if (content.select('.Dropdown-Select__Label.select-option__text')) :
            sizeAll = content.select('.Dropdown-Select__Label.select-option__text')
            for size in sizeAll :
                size = size.text
                tempSizeAll.append(size)
        else :
            tempSizeAll.append("ONE")
        descriptions = content.select('.prod-attr-item')
        descs = []
        ...
        temp.append(title)
        temp.append(mainImage)
        temp.append(originalPrice)
        temp.append(tempSizeAll)
        temp.append(descs)
        csvlist.append(temp)    
